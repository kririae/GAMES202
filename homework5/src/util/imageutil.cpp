...
float *ReadImageLayer(const std::string &filename, const std::string &layername,
  ...
  float *out;  // width * height * RGBA
  const char *err = nullptr;
  int ret = LoadEXRWithLayer(&out, &width, &height, filename.c_str(),
                             layername.c_str(), &err);
  float *buffer = new float[width * height * channel];
...
bool WriteImage(const std::string &filename, const int &width,
                const int &height, const int &channel, const float *buffer) {
  CHECK(channel == 1 || channel == 3);
  EXRHeader header;
