...
Buffer2D<Float3> ReadFloat3Image(const std::string &filename) {
  ...
  for (int i = 0; i < width * height; i++) {
    buffer[i] =
        Float3(_buffer[i * 3 + 0], _buffer[i * 3 + 1], _buffer[i * 3 + 2]);
  }
  delete[] _buffer;
...
void WriteFloatImage(const Buffer2D<float> &imageBuffer,
                     const std::string &filename) {
  WriteImage(filename, imageBuffer.m_width, imageBuffer.m_height, 1,
             (float *)imageBuffer.m_buffer.get());
}

void WriteFloat3Image(const Buffer2D<Float3> &imageBuffer,
                      const std::string &filename) {
