 ...
 * test suites not unlike the traditional unit tests used in software
 * engineering.
 *
 ...
 public:
  StudentsTTest(const PropertyList &propList) {
    ...
    std::vector<std::string> angles =
        tokenize(propList.getString("angles", ""));
    for (auto angle : angles) m_angles.push_back(toFloat(angle));

    /* This parameter specifies a list of reference values, one for each angle
     */
    std::vector<std::string> references =
        tokenize(propList.getString("references", ""));
  ...
  virtual ~StudentsTTest() {
    ...
    for (auto scene : m_scenes) delete scene;
  }
  ...
  void activate() {
    ...
    if (!m_bsdfs.empty()) {
      if (m_references.size() * m_bsdfs.size() != m_angles.size())
        throw NoriException(
            "Specified a different number of angles and reference values!");
      if (!m_scenes.empty())
      ...
      for (auto bsdf : m_bsdfs) {
        for (size_t i = 0; i < m_references.size(); ++i) {
          ...
          cout << "------------------------------------------------------"
               << endl;
          cout << "Testing (angle=" << angle << "): " << bsdf->toString()
               << endl;
          ++total;
          ...
          std::pair<bool, std::string> result = hypothesis::students_t_test(
              mean, variance, reference, m_sampleCount, m_significanceLevel,
      ...
      for (auto scene : m_scenes) {
        ...
        for (int k = 0; k < m_sampleCount; ++k) {
          ...
          Point2f pixelSample = (sampler->next2D().array() *
                                 camera->getOutputSize().cast<float>().array())
                                    .matrix();
          Color3f value =
              camera->sampleRay(ray, pixelSample, sampler->next2D());
  ...
  std::string toString() const {
    return tfm::format(
        ...
        "  sampleCount= %i\n"
        "]",
        m_significanceLevel, m_sampleCount);
  }
