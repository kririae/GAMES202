...
class ChiSquareTest : public NoriObject {
 public:
  ...
  virtual ~ChiSquareTest() {
    for (auto bsdf : m_bsdfs) delete bsdf;
  }
  ...
  void activate() {
    ...
    for (auto bsdf : m_bsdfs) {
      ...
      for (int l = 0; l < m_testCount; ++l) {
        ...
        for (int i = 0; i < m_sampleCount; ++i) {
          ...
          if ((result.array() == 0).all()) continue;

          int cosThetaBin =
              std::min(std::max(0, (int)std::floor((bRec.wo.z() * 0.5f + 0.5f) *
          ...
          int phiBin = std::min(
              std::max(0, (int)std::floor(scaledPhi * m_phiResolution)),
              m_phiResolution - 1);
          ...
          for (int j = 0; j < m_phiResolution; ++j) {
            ...
            auto integrand = [&](double cosTheta, double phi) -> double {
              ...
              Vector3f wo((float)(sinTheta * cosPhi),
                          (float)(sinTheta * sinPhi), (float)cosTheta);
            ...
            double integral = hypothesis::adaptiveSimpson2D(
                integrand, cosThetaStart, phiStart, cosThetaEnd, phiEnd);
        ...
        hypothesis::chi2_dump(m_cosThetaResolution, m_phiResolution,
                              obsFrequencies.get(), expFrequencies.get(),
        ...
        std::pair<bool, std::string> result = hypothesis::chi2_test(
            ...
            expFrequencies.get(), m_sampleCount, m_minExpFrequency,
            m_significanceLevel, m_testCount * (int)m_bsdfs.size());
    ...
    if (passed < total) throw std::runtime_error("Some tests failed :(");
  }
  ...
  std::string toString() const {
    return tfm::format(
        ...
        "  significanceLevel = %f\n"
        "]",
        m_cosThetaResolution, m_phiResolution, m_minExpFrequency, m_sampleCount,
        m_testCount, m_significanceLevel);
  }
