...
Bitmap *ImageBlock::toBitmap() const {
  ...
  for (int y = 0; y < m_size.y(); ++y)
    for (int x = 0; x < m_size.x(); ++x)
      result->coeffRef(y, x) =
          coeff(y + m_borderSize, x + m_borderSize).divideByFilterWeight();
  return result;
}
...
void ImageBlock::put(const Point2f &_pos, const Color3f &value) {
  ...
  bbox.clip(
      BoundingBox2i(Point2i(0, 0), Point2i((int)cols() - 1, (int)rows() - 1)));
...
std::string ImageBlock::toString() const {
  return tfm::format("ImageBlock[offset=%s, size=%s]]", m_offset.toString(),
                     m_size.toString());
}

BlockGenerator::BlockGenerator(const Vector2i &size, int blockSize)
    : m_size(size), m_blockSize(blockSize) {
  m_numBlocks = Vector2i((int)std::ceil(size.x() / (float)blockSize),
...
bool BlockGenerator::next(ImageBlock &block) {
  ...
  do {
    ...
    if (--m_stepsLeft == 0) {
      ...
      if (m_direction == ELeft || m_direction == ERight) ++m_numSteps;
