...
NoriObject *loadFromXML(const std::string &filename) {
  ...
  auto check_attributes = [&](const pugi::xml_node &node,
    ...
    for (auto attr : node.attributes()) {
      ...
      if (it == attrs.end())
        throw NoriException(
            "Error while parsing \"%s\": unexpected attribute \"%s\" in \"%s\" "
            "at %s",
    ...
    if (!attrs.empty())
      throw NoriException(
          "Error while parsing \"%s\": missing attribute \"%s\" in \"%s\" at "
          "%s",
  ...
  std::function<NoriObject *(pugi::xml_node &, PropertyList &, int)> parseTag =
    ...
    if (node.type() != pugi::node_element)
      throw NoriException(
          "Error while parsing \"%s\": unexpected content at %s", filename,
    ...
    if (it == tags.end())
      throw NoriException(
          "Error while parsing \"%s\": unexpected tag \"%s\" at %s", filename,
          node.name(), offset(node.offset_debug()));
    int tag = it->second;

    /* Perform some safety checks to make sure that the XML tree really makes
     * sense */
    ...
    if (!hasParent && !currentIsObject)
      throw NoriException(
          "Error while parsing \"%s\": root element \"%s\" must be a Nori "
          "object (at %s)",
    ...
    if (hasParent && !parentIsObject &&
      ...
      throw NoriException(
          "Error while parsing \"%s\": node \"%s\" requires a Nori object as "
          "parent (at %s)",
          filename, node.name(), offset(node.offset_debug()));
    ...
    try {
      if (currentIsObject) {
        ...
        switch (tag) {
          ...
          case EBoolean: {
            check_attributes(node, {"name", "value"});
            list.setBoolean(node.attribute("name").value(),
                            toBool(node.attribute("value").value()));
          } break;
          case EPoint: {
            check_attributes(node, {"name", "value"});
            list.setPoint(node.attribute("name").value(),
                          Point3f(toVector3f(node.attribute("value").value())));
          } break;
          case EVector: {
            check_attributes(node, {"name", "value"});
            list.setVector(
                node.attribute("name").value(),
                Vector3f(toVector3f(node.attribute("value").value())));
          } break;
          case EColor: {
            check_attributes(node, {"name", "value"});
            list.setColor(
                node.attribute("name").value(),
                Color3f(toVector3f(node.attribute("value").value()).array()));
          } break;
          case ETransform: {
          ...
          case ETranslate: {
            ...
            transform =
                Eigen::Translation<float, 3>(v.x(), v.y(), v.z()) * transform;
          } break;
          case EMatrix: {
            check_attributes(node, {"value"});
            std::vector<std::string> tokens =
                tokenize(node.attribute("value").value());
            if (tokens.size() != 16) throw NoriException("Expected 16 values");
          ...
          case ELookAt: {
            ...
            Eigen::Vector3f origin =
                toVector3f(node.attribute("origin").value());
            Eigen::Vector3f target =
                toVector3f(node.attribute("target").value());
