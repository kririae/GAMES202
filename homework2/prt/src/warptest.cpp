...
enum PointType : int { Independent = 0, Grid, Stratified };
...
static const std::string kWarpTypeNames[WarpTypeCount] = {"square",
                                                          "tent",
                                                          "disk",
                                                          "uniform_sphere",
                                                          "uniform_hemisphere",
...
struct WarpTest {
  ...
  WarpTest(WarpType warpType_, float parameterValue_, BSDF *bsdf_ = nullptr,
      ...
      : warpType(warpType_),
        ...
        xres(xres_),
        yres(yres_) {
    if (warpType != Square && warpType != Disk && warpType != Tent) xres *= 2;
    res = xres * yres;
  }
  ...
  std::pair<bool, std::string> run() {
    ...
    auto integrand = [&](double y, double x) -> double {
      if (warpType == Square) {
        ...
        double sinPhi = std::sin(x), cosPhi = std::cos(x);

        nori::Vector3f v((float)(sinTheta * cosPhi), (float)(sinTheta * sinPhi),
                         (float)y);
    ...
    for (int y = 0; y < yres; ++y) {
      ...
      for (int x = 0; x < xres; ++x) {
        ...
        ptr[y * xres + x] = hypothesis::adaptiveSimpson2D(integrand, yStart,
                                                          xStart, yEnd, xEnd) *
                            scale;
        if (ptr[y * xres + x] < 0)
    ...
    hypothesis::chi2_dump(yres, xres, obsFrequencies.get(),
                          expFrequencies.get(), "chitest.m");
  ...
  std::pair<Point3f, float> warpPoint(const Point2f &sample) {
    ...
    switch (warpType) {
      ...
      case MicrofacetBRDF: {
        ...
        return std::make_pair(br.wo, value == 0 ? 0.f : bsdf->eval(br)[0]);
      }
      default:
  ...
  static std::pair<BSDF *, BSDFQueryRecord> create_microfacet_bsdf(
      float alpha, float kd, float bsdfAngle) {
    PropertyList list;
...
struct Arcball {
  ...
  Arcball(float speedFactor = 2.0f)
      : m_active(false),
        m_lastPos(nori::Vector2i::Zero()),
        m_size(nori::Vector2i::Zero()),
  ...
  void button(nori::Vector2i pos, bool pressed) {
    m_active = pressed;
    m_lastPos = pos;
    if (!m_active) m_quat = (m_incr * m_quat).normalized();
    m_incr = Quaternionf::Identity();
  }
 ...
 private:
  ...
  /// The last click position (which triggered the Arcball to be active /
  /// non-active).
  ...
  /// When active, tracks the overall update to the state.  Identity when
  /// non-active.
  Quaternionf m_incr;
...
class WarpTestScreen : public Screen {
 public:
  WarpTestScreen()
      : Screen(Vector2i(800, 600), "warptest: Sampling and Warping"),
        m_bRec(nori::Vector3f()) {
    inc_ref();
    initializeGUI();
  ...
  void refresh() {
    ...
    if (warpType != Square) {
      for (int i = 0; i < m_pointCount; ++i) {
        if (values(0, i) == 0.0f) {
          positions.col(i) =
              nori::Vector3f::Constant(std::numeric_limits<float>::quiet_NaN());
          continue;
        }
        positions.col(i) =
    ...
    m_pointShader->set_buffer("position", VariableType::Float32,
                              {(size_t)m_pointCount, 3}, positions.data());
    m_pointShader->set_buffer("color", VariableType::Float32,
                              {(size_t)m_pointCount, 3}, colors.data());
    ...
    if (m_gridCheckBox->checked()) {
      ...
      for (int i = 0; i <= gridRes; ++i) {
        for (int j = 0; j <= fineGridRes; ++j) {
          ...
          positions.col(idx++) = value_scale == 0.f
                                     ? pt.first
                                     : (pt.first * pt.second * value_scale);
      ...
      m_gridShader->set_buffer("position", VariableType::Float32,
                               {(size_t)m_lineCount, 3}, positions.data());
    }
    ...
    for (int i = 0; i <= 50; ++i) {
      ...
      positions.col(ctr++) << std::cos(angle1) * .5f + 0.5f,
          std::sin(angle1) * .5f + 0.5f, 0.f;
      positions.col(ctr++) << std::cos(angle2) * .5f + 0.5f,
          std::sin(angle2) * .5f + 0.5f, 0.f;
    }
    positions.col(ctr++) << 0.5f, 0.5f, 0.f;
    positions.col(ctr++) << -m_bRec.wi.x() * 0.5f + 0.5f,
        -m_bRec.wi.y() * 0.5f + 0.5f, m_bRec.wi.z() * 0.5f;
    positions.col(ctr++) << 0.5f, 0.5f, 0.f;
    positions.col(ctr++) << m_bRec.wi.x() * 0.5f + 0.5f,
        m_bRec.wi.y() * 0.5f + 0.5f, m_bRec.wi.z() * 0.5f;
    ...
    m_angleBox->set_value(
        tfm::format("%.1f", m_angleSlider->value() * 180 - 90));
    m_parameterSlider->set_enabled(warpType == Beckmann ||
                                   warpType == MicrofacetBRDF);
    m_parameterBox->set_enabled(warpType == Beckmann ||
                                warpType == MicrofacetBRDF);
    ...
    m_pointCountSlider->set_value(
        (std::log((float)m_pointCount) / std::log(2.f) - 5) / 15);
  }
  ...
  void draw_contents() {
    ...
    view = Matrix4f::look_at(Vector3f(0, 0, 4), Vector3f(0, 0, 0),
                             Vector3f(0, 1, 0));
    ...
    if (m_drawHistogram) {
      ...
      const int histHeight =
          (warpType == Square || warpType == Disk || warpType == Tent)
              ? histWidth
              : histWidth / 2;
      const int verticalOffset = (height() - histHeight) / 2;

      drawHistogram(Vector2i(spacer, verticalOffset),
                    Vector2i(histWidth, histHeight), m_textures[0].get());
      drawHistogram(Vector2i(2 * spacer + histWidth, verticalOffset),
                    Vector2i(histWidth, histHeight), m_textures[1].get());

      auto ctx = m_nvg_context;
      nvgBeginFrame(ctx, m_size[0], m_size[1], m_pixel_ratio);
      nvgBeginPath(ctx);
      nvgRect(ctx, spacer, verticalOffset + histHeight + spacer,
              width() - 2 * spacer, 70);
      nvgFillColor(ctx, m_testResult.first ? Color(100, 255, 100, 100)
      ...
      nvgText(ctx, 2 * spacer + (histWidth * 3) / 2,
              verticalOffset - 3 * spacer, "Integrated density", nullptr);
      ...
      nvgTextBox(ctx, spacer,
                 verticalOffset + histHeight + spacer + (70 - bounds[3]) / 2,
      ...
      m_pointShader->begin();
      m_pointShader->draw_array(nanogui::Shader::PrimitiveType::Point, 0,
                                m_pointCount);
      m_pointShader->end();
      ...
      if (drawGrid) {
        ...
        m_gridShader->begin();
        m_gridShader->draw_array(nanogui::Shader::PrimitiveType::Line, 0,
                                 m_lineCount);
        m_gridShader->end();
      }
  ...
  void drawHistogram(const nanogui::Vector2i &pos_,
                     const nanogui::Vector2i &size_, Texture *texture) {
    ...
    m_histogramShader->draw_array(nanogui::Shader::PrimitiveType::Triangle, 0,
                                  6, true);
  ...
  void runTest() {
    ...
    for (int i = 0; i < tester.res; ++i) {
      maxValue = std::max(maxValue, (float)std::max(tester.obsFrequencies[i],
                                                    tester.expFrequencies[i]));
      minValue = std::min(minValue, (float)std::min(tester.obsFrequencies[i],
                                                    tester.expFrequencies[i]));
    }
    minValue /= 2;
    ...
    for (int k = 0; k < 2; ++k) {
      for (int i = 0; i < tester.res; ++i)
        buffer[i] =
            ((k == 0 ? tester.obsFrequencies[i] : tester.expFrequencies[i]) -
             minValue) *
            texScale;
  ...
  void initializeGUI() {
    ...
    m_pointTypeBox =
        new ComboBox(m_window, {"Independent", "Grid", "Stratified"});
    m_pointTypeBox->set_callback([&](int) { refresh(); });

    new Label(m_window, "Warping method", "sans-bold");
    m_warpTypeBox = new ComboBox(
        m_window, {"Square", "Tent", "Disk", "Sphere", "Hemisphere (unif.)",
    ...
    m_pointShader = new Shader(m_renderPass, "Point shader",
    ...
    m_gridShader = new Shader(m_renderPass, "Grid shader",
    ...
    m_arrowShader = new Shader(m_renderPass, "Arrow shader",
    ...
    m_histogramShader = new Shader(m_renderPass, "Histogram shader",
    ...
    m_histogramShader->set_buffer("indices", VariableType::UInt32, {3 * 2},
                                  indices);
    m_histogramShader->set_buffer("position", VariableType::Float32, {4, 2},
                                  positions);
 ...
 private:
  nanogui::ref<Shader> m_pointShader, m_gridShader, m_histogramShader,
      m_arrowShader;
  Window *m_window;
  Slider *m_pointCountSlider, *m_parameterSlider, *m_parameter2Slider,
      *m_angleSlider;
...
std::tuple<WarpType, float, float> parse_arguments(int argc, char **argv) {
  ...
  for (int i = 0; i < WarpTypeCount; ++i) {
    if (strcmp(kWarpTypeNames[i].c_str(), argv[1]) == 0) tp = WarpType(i);
  }
  if (tp >= WarpTypeCount) throw std::runtime_error("Invalid warp type!");

  float value = 0.f, value2 = 0.f;
  if (argc > 2) value = std::stof(argv[2]);
  if (argc > 3) value2 = std::stof(argv[3]);
...
int main(int argc, char **argv) {
  ...
  if (warpType == MicrofacetBRDF) {
    ...
    BSDF *ptr;
    std::tie(ptr, bRec) =
        WarpTest::create_microfacet_bsdf(paramValue, param2Value, bsdfAngle);
    bsdf.reset(ptr);
  }
  ...
  std::cout << tfm::format("Testing warp %s, parameter value = %f%s",
