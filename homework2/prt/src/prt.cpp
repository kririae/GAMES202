...
std::vector<std::unique_ptr<float[]>> LoadCubemapImages(
  ...
  std::vector<std::string> cubemapNames{
      "negx.jpg", "posx.jpg", "posy.jpg", "negy.jpg", "posz.jpg", "negz.jpg"};
...
float CalcArea(const float &u_,
               ...
               const int &width,
               const int &height) {
  // Provided function, map (u, v) -> area of the corresponding space on sphere
...
std::vector<Eigen::Array3f> PrecomputeCubemapSH(
    const std::vector<std::unique_ptr<float[]>> &images,
    const int &width,
    const int &height,
    const int &channel) {
...
public:
  ...
  virtual void preprocess(const Scene *scene) override {
    ...
    std::vector<std::unique_ptr<float[]>> images = ProjEnv::LoadCubemapImages(
        cubePath.str(), width, height, channel);
    auto envCoeffs = ProjEnv::PrecomputeCubemapSH<SHOrder>(
    ...
    for (int i = 0; i < mesh->getVertexCount(); i++) {
      ...
      auto shFunc       = [&](double phi, double theta) -> double {
        ...
        if (m_Type == Type::Unshadowed) {
          // TODO: here you need to calculate unshadowed transport term of a
          // given direction
