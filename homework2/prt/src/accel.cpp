...
void Accel::build() {
  if (m_num_meshes == 0)
    throw NoriException(
        "No mesh found, could not build acceleration structure");
  ...
  for (uint32_t current_mesh_idx = 0; current_mesh_idx < m_num_meshes;
       current_mesh_idx++) {
    uint32_t num_triangles_mesh =
        m_meshes[current_mesh_idx]->getTriangleCount();
...
bool Accel::rayIntersect(const Ray3f& ray_, Intersection& its,
  ...
  if (shadowRay) return foundIntersection;
...
Accel::Node* Accel::buildRecursive(const BoundingBox3f& bbox,
  ...
  // create leaf node if 10 or less triangles are left or if the max recursion
  // depth is reached.
  if (num_triangles <= MAX_TRIANGLES_PER_NODE ||
  ...
  for (uint32_t i = 0; i < 8; i++) {
    ...
    for (uint32_t j = 0; j < num_triangles; j++) {
      ...
      BoundingBox3f triangle_bbox =
          m_meshes[mesh_idx]->getBoundingBox(triangle_idx);

      // check if triangle is in bbox, if so put triangle index into triangle
      // list of child
  ...
  for (uint32_t i = 0; i < 8; i++) {
    // first child
    if (i == 0) {
      node->child = buildRecursive(child_bboxes[i], child_triangle_indices[i],
                                   child_mesh_indices[i], recursion_depth + 1);
      last_child = node->child;
      // neighbour children
    } else {
      last_child->next =
          buildRecursive(child_bboxes[i], child_triangle_indices[i],
                         child_mesh_indices[i], recursion_depth + 1);
...
bool Accel::traverseRecursive(const Node& node, Ray3f& ray, Intersection& its,
                              bool shadowRay, uint32_t& hit_idx) const {
  bool foundIntersection = false;

  // only check triangles of node and its children if ray intersects with node
  // bbox
  ...
  for (uint32_t i = 0; i < node.num_triangles; ++i) {
    ...
    if (m_meshes[mesh_idx]->rayIntersect(triangle_idx, ray, u, v, t) &&
        t < ray.maxt) {
  ...
  if (node.child) {
    ...
    int i = 0;
    do {
      children[i] = std::pair<Node*, float>(
          current_child, current_child->bbox.distanceTo(ray.o));
      current_child = current_child->next;
      i++;
    } while (current_child);

    std::sort(children, children + 8,
              [ray](const std::pair<Node*, float>& l,
                    const std::pair<Node*, float>& r) {
                return l.second < r.second;
              });

    for (auto child : children) {
      foundIntersection =
          traverseRecursive(*child.first, ray, its, shadowRay, hit_idx) ||
          foundIntersection;
...
void Accel::subdivideBBox(const nori::BoundingBox3f& parent,
                          nori::BoundingBox3f* bboxes) {
  Point3f extents = parent.getExtents();

  Point3f x0_y0_z0 = parent.min;
  Point3f x1_y0_z0 = Point3f(parent.min.x() + extents.x() / 2.f, parent.min.y(),
                             parent.min.z());
  Point3f x0_y1_z0 = Point3f(parent.min.x(), parent.min.y() + extents.y() / 2.f,
                             parent.min.z());
  Point3f x1_y1_z0 =
      Point3f(parent.min.x() + extents.x() / 2.f,
              parent.min.y() + extents.y() / 2.f, parent.min.z());

  Point3f x0_y0_z1 = Point3f(parent.min.x(), parent.min.y(),
                             parent.min.z() + extents.z() / 2.f);
  Point3f x1_y0_z1 = Point3f(parent.min.x() + extents.x() / 2.f, parent.min.y(),
                             parent.min.z() + extents.z() / 2.f);
  Point3f x0_y1_z1 = Point3f(parent.min.x(), parent.min.y() + extents.y() / 2.f,
                             parent.min.z() + extents.z() / 2.f);
  Point3f x1_y1_z1 = Point3f(parent.min.x() + extents.x() / 2.f,
                             parent.min.y() + extents.y() / 2.f,
                             parent.min.z() + extents.z() / 2.f);
  Point3f x2_y1_z1 = Point3f(parent.max.x(), parent.min.y() + extents.y() / 2.f,
                             parent.min.z() + extents.z() / 2.f);
  Point3f x1_y2_z1 = Point3f(parent.min.x() + extents.x() / 2.f, parent.max.y(),
                             parent.min.z() + extents.z() / 2.f);
  Point3f x2_y2_z1 = Point3f(parent.max.x(), parent.max.y(),
                             parent.min.z() + extents.z() / 2.f);

  Point3f x1_y1_z2 =
      Point3f(parent.min.x() + extents.x() / 2.f,
              parent.min.y() + extents.y() / 2.f, parent.max.z());
  Point3f x2_y1_z2 = Point3f(parent.max.x(), parent.min.y() + extents.y() / 2.f,
                             parent.max.z());
  Point3f x1_y2_z2 = Point3f(parent.min.x() + extents.x() / 2.f, parent.max.y(),
                             parent.max.z());
