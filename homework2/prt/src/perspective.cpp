...
class PerspectiveCamera : public Camera {
 public:
  ...
  void activate() {
    ...
    perspective << cot, 0, 0, 0, 0, cot, 0, 0, 0, 0, m_farClip * recip,
        -m_nearClip * m_farClip * recip, 0, 0, 1, 0;
     ...
     */
    m_sampleToCamera =
        Transform(Eigen::DiagonalMatrix<float, 3>(
  ...
  Color3f sampleRay(Ray3f &ray, const Point2f &samplePosition,
    ...
    Point3f nearP = m_sampleToCamera *
                    Point3f(samplePosition.x() * m_invOutputSize.x(),
  ...
  std::string toString() const {
    return tfm::format(
        ...
        "  rfilter = %s\n"
        "]",
        indent(m_cameraToWorld.toString(), 18), m_outputSize.toString(), m_fov,
        m_nearClip, m_farClip, indent(m_rfilter->toString()));
  }
