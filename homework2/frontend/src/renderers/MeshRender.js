  ...
  bindMaterialParameters() {
    ...
    for (let k in this.material.uniforms) {
      if (this.material.uniforms[k].type == "matrix4fv") {
        ...
        gl.uniform1i(this.shader.program.uniforms[k], textureNum);
        textureNum += 1;
      } else if (this.material.uniforms[k].type == "precomputeLR") {
        let envMat3 = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
        gl.uniformMatrix3fv(this.shader.program.uniforms[k], false, envMat3[0]);
      } else if (this.material.uniforms[k].type == "precomputeLG") {
        let envMat3 = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
        gl.uniformMatrix3fv(this.shader.program.uniforms[k], false, envMat3[1]);
      } else if (this.material.uniforms[k].type == "precomputeLB") {
        let envMat3 = getMat3ValueFromRGB(precomputeL[guiParams.envmapId]);
        gl.uniformMatrix3fv(this.shader.program.uniforms[k], false, envMat3[2]);
      }
    }
  }
