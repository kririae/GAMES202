...
samplePoints squareToCosineHemisphere(int sample_count) {
  ...
  for (int t = 0; t < sample_side; t++) {
    for (int p = 0; p < sample_side; p++) {
      ...
      Vec3f wi =
          Vec3f(sin(theta) * cos(phi), sin(theta) * sin(phi), cos(theta));
      float pdf = wi.z / PI;
...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
  ...
  float B = 0.0;
  float C = 0.0;
  const int sample_count = 1024 * 32;
  ...
  for (int i = 0; i < sample_count; i++) {
    ...
    // auto sample = [=]() {
    // const float R_0 = 0.0f;
    // const float F = R_0 + (1 - R_0) * std::pow(1 - std::cos(NdotV), 5.0f);
    const float F = 1.0f;
    const float D = DistributionGGX(N, normalize(L + V), roughness);
    const float G = GeometrySmith(roughness, NdotV, dot(N, L));
    const float M = 4.0f * NdotV;  // cos
    float res = G / M * F * D / pdf;
    // };
...
int main() {
  ...
  stbi_write_png("GGX_E_MC_LUT.png", resolution, resolution, 3, data,
                 resolution * 3);
