...
Vec3f getEmu(int x, int y, int alpha, unsigned char *data, float NdotV,
             float roughness) {
...
Vec3f IntegrateEmu(Vec3f V, float roughness, float NdotV, Vec3f Ei) {
  ...
  static samplePoints sampleList = squareToCosineHemisphere(sample_count);
...
int main() {
  unsigned char *Edata =
      stbi_load("./GGX_E_MC_LUT.png", &resolution, &resolution, &channel, 3);
  if (Edata == NULL) {
    ...
    stbi_write_png("GGX_Eavg_LUT.png", resolution, resolution, channel, data,
                   0);
  }
